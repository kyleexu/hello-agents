...
async def search_poi(
    ...
    try:
        ...
        print(f"❌ POI搜索失败: {e!s}")
        raise HTTPException(status_code=500, detail=f"POI搜索失败: {e!s}")
...
async def get_weather(city: str = Query(..., description="城市名称", example="北京")):
    ...
    try:
        ...
        print(f"❌ 天气查询失败: {e!s}")
        raise HTTPException(status_code=500, detail=f"天气查询失败: {e!s}")
...
async def plan_route(request: RouteRequest):
    ...
    try:
        ...
        print(f"❌ 路线规划失败: {e!s}")
        raise HTTPException(status_code=500, detail=f"路线规划失败: {e!s}")
...
async def health_check():
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"服务不可用: {e!s}")
