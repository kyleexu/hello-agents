...
async def get_poi_detail(poi_id: str):
    ...
    try:
        ...
        print(f"❌ 获取POI详情失败: {e!s}")
        raise HTTPException(status_code=500, detail=f"获取POI详情失败: {e!s}")
...
async def search_poi(keywords: str, city: str = "北京"):
    ...
    try:
        ...
        print(f"❌ 搜索POI失败: {e!s}")
        raise HTTPException(status_code=500, detail=f"搜索POI失败: {e!s}")
...
async def get_attraction_photo(name: str):
    ...
    try:
        ...
        print(f"❌ 获取景点图片失败: {e!s}")
        raise HTTPException(status_code=500, detail=f"获取景点图片失败: {e!s}")
